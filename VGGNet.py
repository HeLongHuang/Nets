...
def getData():  # 定义数据预处理
    ...
    trainset = tv.datasets.CIFAR10(root='./Cifar-10', train=True, transform=transform, download=True)  # 获取CIFAR10的训练数据
    testset = tv.datasets.CIFAR10(root='./Cifar-10', train=False, transform=transform, download=True)  # 获取CIFAR10的测试数据
...
def train():
    ...
    for epoch in range(EPOCH):
        ...
        for step, (inputs_cpu, labels_cpu) in enumerate(trainset_loader):
            ...
            _, predicted = torch.max(output, 1)  # predicted 当前图像预测的类别 "_"表示的是原来的数据  predicted表示的是最大数值的下表  也就是类别的意思
            sum_loss += loss.item()
            total += labels.size(0)
            accuracy += (predicted == labels).sum()
            t2 = time.time()
            # tensor数据(在GPU上计算的)如果需要进行常规计算，必须要加.cpu().numpy()转换为numpy类型，否则数据类型无法自动转换为float
            print("epoch %d | step %d: loss = %.4f, the accuracy now is %.3f %%, time = %f" % (epoch, step, sum_loss/(step+1), 100.*accuracy.cpu().numpy()/total,t2 - t1))
...
def test(net, testdata):
    ...
    return float(correct.numpy()) / total
